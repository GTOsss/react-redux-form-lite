...
import SyncValidation from './sync-validation-object-model';
...
describe('<SyncValidationObjectModel />', () => {
  ...
  test('validate: onSubmit after failed submit', () => {
    ...
    expect(onSubmit.mock.calls[0][0].values).toEqual({profile: {firstName: 'test', lastName: undefined}});
    ...
    expect(onSubmit.mock.calls[0][0].state.form.errorsMap).toEqual({profile: {lastName: 'Field required.'}});
  ...
  test('validate: onSubmit after failed and success', () => {
    ...
    expect(onSubmit.mock.calls[1][0].values).toEqual({profile: {firstName: 'test', lastName: 'test'}});
  ...
  test('warn: onSubmit after failed submit', () => {
    ...
    component.find('form').simulate('submit');
    expect(onSubmit.mock.calls.length).toBe(1);
    expect(onSubmit.mock.calls[0][0].values).toEqual({profile: {firstName: 't', lastName: 'test'}});
